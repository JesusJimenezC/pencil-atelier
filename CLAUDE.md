...
- `/pencil-design` (`skills/pencil-design/SKILL.md`) - authors new `.pen` content already compliant with the rules; loads `design.md` on Step A if present. Enforces the safe-apply discipline (no `replace_all_matching_properties` for `$`-values).
...
- `pencil-auditor` (`agents/pencil-auditor.md`) - read-only auditor. Reports divergences; never mutates. Checks R5 (corruption) first, then R1/R3/R2.
...
### Five house rules (shared vocabulary across skills/agents)

All skills and the auditor reference the same rule IDs. If you rename or renumber, update every file.

**Rule priority when multiple rules flag the same node: R5 > R1 > R3 > R2.** R5 takes precedence because it describes a broken render, not a style choice.
...
- **R5** - variable-reference integrity (no `\$X` strings; canonical `$X` only). Release blocker. Detected by `pencil-auditor`, repaired by explicit `batch_design` U() ops (never `replace_all_matching_properties`). The root cause is `replace_all_matching_properties` escaping `$` into `\$` during bulk promotion — `batch_design` U() preserves `$` literally and is the only safe write path for `$`-values.

### Safe-apply discipline (v0.1.1+)

`replace_all_matching_properties` is **forbidden** for variable promotion anywhere in the plugin. It escapes `$` into `\$`, producing the exact R5 corruption this plugin now detects. When editing skills or agents:

- Do NOT recommend `replace_all_matching_properties` for `to:` values that begin with `$`.
- Do NOT recommend it even as a "faster path" for bulk R1 promotion — the corruption it produces is silent and only surfaces as black-fallback rendering.
- All write paths (R5 repair, R1 promotion, ad-hoc token fixes) MUST go through `batch_design` with explicit `U(nodeId, {prop: "$token"})` ops.
- For nested properties (`stroke`, `effects[*].shadow`, etc.), pass the full object — `U(id, { stroke: { fill: "$X", thickness: 1 } })` — not dot-path keys like `"stroke.fill"`, which silently no-op on some shapes.
- After any batch that touches `$`-values, re-run `batch_get` on the written nodes and confirm zero `\$` strings remain. Screenshot the root frame. Silent success in the API does not prove the design renders.
...
All three subagents (`pencil-auditor`, `pencil-navigator`, `pencil-analyzer`) are declared read-only in their frontmatter `tools:` list - none has access to `batch_design`, `set_variables`, `image`, `Write`, or `Edit`, and **none has access to `replace_all_matching_properties`** either (its inclusion would be a bug per the safe-apply discipline). When adding capabilities to a subagent, preserve this boundary: write operations (both `.pen` mutation and `design.md` authoring) belong in the calling skill (main agent), not in subagents.
...
- Rule IDs (R1/R2/R3/R4/R5) referenced consistently across files.
- Rule priority **R5 > R1 > R3 > R2** stated identically in `pencil-auditor.md`, `pencil-audit/SKILL.md`, and this file.
...
- `disable-model-invocation: true` is preserved on all five skills.
...
- No file recommends `replace_all_matching_properties` for any `to:` value beginning with `$`. Safe-apply discipline is non-negotiable.
- When `/pencil-audit` reports any R5 blocker, its Recommendations section must include an inline `batch_design` U() recipe, never a recommendation of `replace_all_matching_properties`.
